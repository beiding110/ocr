...
for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
        ...
        if (
            Math.abs(data[index] - data[index + 1]) > 10 &&
            Math.abs(data[index + 1] - data[index + 2]) > 10 &&
            Math.abs(data[index] - data[index + 2]) > 10
        ) {
            // increase the count by 1 (or set it to 1 if the color wasn't there yet)
            colorOccurrences[color] = {
                pos: [x, y],
                count: (colorOccurrences[color]?.count || 0) + 1,
            };
        }
    }
}
...
let highestColor = colors.reduce((highColor, currentColor) => {
    if (highColor[1].count > currentColor[1].count) {
        return highColor;
    } else {
        return currentColor;
    }
